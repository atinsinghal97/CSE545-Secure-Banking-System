...
public class LoginController {
    ...
    public ModelAndView forgotPassword(final HttpServletRequest request,
        ...
        try {
            ...
            if (u != null) {
                Boolean emailPending = s.createNamedQuery("FindPendingTransactionsByUser", Boolean.class)
                        .setParameter("user_id", u.getId()).setParameter("offset", OtpUtils.EXPIRE_TIME).uniqueResult();

                if (emailPending) {
                    model.put("message",
                            "Password reset instructions has already been sent to the email/phone associated with this account. Please wait for some more time before requesting another reset link.");
                    response.setViewName("ForgotPassword");
                    return response;
                }
                ...
                Application.executorService.submit(() -> {
                    ...
                    try {
                        ...
                        if (mode == 1) {
                            ...
                            mailer.sendEmail(details.getEmail(), "BigPPBank: Your link to reset your password.",
        ...
        if (mode == 0) {
            model.put("message",
                    "If your username exists in the database, you'll recieve a message to reset your password.");
            response.setViewName("ForgotPassword");
        } else {
    ...
    public ModelAndView resetPassword(final HttpServletRequest request, Model model,
        ...
        // This will invalidate the OTP no matter what
        User u = otpUtils.validateOtp(otp, request.getRemoteAddr());

        if (u != null) {
            Authentication auth = new UsernamePasswordAuthenticationToken(new UserDetailsImpl(u), null,
